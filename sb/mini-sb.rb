# bootstrap: parse spellbreak lex/peg rules, and translate to action calls.

# we name the recursive call of compile functions the "AST"

# to save some work...:
#   - calbacks are dealt with mini-callback
#   - no need to preserve space.pre-callback and space.pre-eq
#   - no need to parse `/+`, `/?`
#   - no need to parse rules other than `lex` and `peg`
# but:
#   - still need to parse `/*`
#   - still need to duplicate AST calls of `peg Lex` and `peg Peg`

require_relative "mini-lex"
require_relative "mini-peg"

class MiniSb

  StructIns = Struct.new :name, :args
  class StructIns
    def eval
      Klasses.validate self.class
      name_tok = Token.new 'name.struct', name
      args_list = args.map {|arg| Token.new('name.arg', arg).eval }
      "NODE(StructIns, 2, #{name_tok.eval}, #{build_list args_list})"
    end
  end

  Main = Struct.new :ins
  class Main
    def eval
      Klasses.validate self.class
      multi_ins = build_list ins.map &:eval
      "NODE(Main, 1, #{multi_ins})"
    end
  end

  def initialize src
    src = src.gsub(/(?<!\\)#[^\n]*/, '').gsub(/\ +$/, '')
    @lex = {}
    @peg = {}
    @structs = {}
    curr = nil
    src.lines.each_with_index do |line, i|
      if line =~ /^lex (\*?\w+)\[/
        curr = (@lex[$1] = '')
      elsif line =~ /^peg (\w+)\[/
        curr = (@peg[$1] = '')
      elsif line.strip == "]"
        curr = nil
      elsif line =~ /^struct (\w+)\[(.*?)\]/
        name = $1
        params = $2
        if Klasses[name]
          raise "duplicate struct def at #{i + 1}: #{name}"
        end
        params = params.to_s.split(/, */)
        Klasses.add name, params
        @structs[name] = params
        curr = nil
      elsif curr
        curr << line
      elsif line.strip.empty?
        # blank line
      else
        raise "don't know what to do with line: #{line}"
      end
    end
  end
  attr_reader :lex, :peg

  def parse
    ins = []
    @structs.each do |name, params|
      ins << StructIns.new(name, params)
    end
    @lex.each do |ctx, src|
      ins << MiniLex.new(ctx, src).parse rescue (puts src; raise)
    end
    @peg.each do |ctx, src|
      ins << MiniPeg.new(ctx, src).parse rescue (puts src; raise)
    end
    Main.new ins
  end

  # check sliced string
  def debug_slice member
    send(member).each do |ctx, content|
      puts ctx
      puts content
      puts
    end
  end
end

if __FILE__ == $PROGRAM_NAME
  require "pp"
  parser = MiniSb.new(File.read __dir__ + '/sb.sb')
  node = parser.parse
  # parser.debug_slice :peg
  # parser.debug_slice :lex

  File.open __dir__ + "/sb-klasses.inc", 'w' do |f|
    f.puts "// GENERATED BY mini-sb.rb WITH sb.sb"
    f.puts
    Klasses.to_a.sort_by(&:first).each do |(name, fields)|
      f.puts "DEF_NODE(#{name}, #{fields.map(&:inspect).join ', '});"
    end
  end
  puts "defined #{Klasses.size} structs"

  ast = node.eval
  out_file = __dir__ + "/bootstrap.inc"
  File.open out_file, 'w' do |f|
    f.puts "// GENERATED BY mini-sb.rb WITH sb.sb"
    f.puts
    f.puts "return #{ast};"
  end

  # report actions
  actions = MiniCallback::Call.funcs.uniq.sort.map{|(f, arity)| f.sub ':', '' }
  impl_actions = File.read(__dir__ + '/sb.c').scan(/^static Val (\w+)\(Spellbreak\*/).map{|(f)| f }
  if !(actions - impl_actions).empty?
    puts
    puts "not in impl actions:"
    puts "-" * 70
    puts (actions - impl_actions)
  end
  if !(impl_actions - actions).empty?
    puts
    puts "redundant impl action:"
    puts "-" * 70
    puts (impl_actions - actions)
  end
end
